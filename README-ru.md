...
---
<h2> UX rework версия</h2>

Эта версия форка делает GTweak понятнее перед применением твиков. Логика тумблеров переработана так, чтобы интерфейс показывал состояние самого твика: зелёный означает, что твик применён, серый — что твик не применён. В панели описания теперь есть отдельная подсказка для каждой вкладки: службы, приватность, интерфейс и системные настройки объясняются своим текстом. Цель — убрать старую неоднозначность, когда включённый тумблер можно было прочитать и как включение функции Windows, и как применение твика, который эту функцию отключает.
